...
namespace ConsoleTest
{
    ...
    public static class Profiler
    {
        ...
        public static void RunProcess(Filter filter, Action<Action<StringDictionary, ProfilerRuntime>> process,
            Action<int, byte[]> onCallback, Action<Exception> onError, Action onComplete)
        {
            ...
            var includes = string.IsNullOrWhiteSpace(filter.Includes)
                ? String.Empty.ToCharArray()
                : filter.Includes.ToCharArray();
            var excludes = string.IsNullOrWhiteSpace(filter.Excludes)
                ? String.Empty.ToCharArray()
                : filter.Excludes.ToCharArray();

            const int pfs = (32768*4) + 4 + 4;

            using (
                var mmf2 = MemoryMappedFile.CreateOrOpen("Local\\MMProfilerMapBuffer" + tag,
                    4 + filter.BufferSize + filter.ThresholdSize))
            {
                using (var mmf = MemoryMappedFile.CreateOrOpen("Local\\MMProfilerMapControl" + tag, pfs))
                {
                    ...
                    var streamAccessor = mmf2.CreateViewStream(4, filter.BufferSize + filter.ThresholdSize,
                    ...
                    ThreadPool.QueueUserWorkItem((state) =>
                    {
                        try
                        {
                            process((dictionary, runtime) =>
                            {
                                ...
                                if (dictionary.ContainsKey("Cor_Profiler") &&
                                    dictionary.ContainsKey("Cor_Enable_Profiling") &&
                                    dictionary["Cor_Enable_Profiling"] == "1")
                                {
                                    Console.WriteLine("WARNING A PROFILER IS ALREADY REGISTERED!!!");
                                }
                                else
                                {
